...
namespace BookShop.Core.Features.Discount.Commands.Validations
{
    public class AddDiscountValidator : AbstractValidator<AddDiscountCommand>
    {
        ...
        #region Constructors
        public AddDiscountValidator(IDiscountService discountService, IStringLocalizer<SharedResources> localizer)
        {
        ...
        {
            RuleFor(x => x.Name_Ar)
                .MustAsync(async (Key, CancellationToken) => !await _discountService.IsNameArExist(Key))
                .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
            RuleFor(x => x.Name)
               .MustAsync(async (Key, CancellationToken) => !await _discountService.IsNameExist(Key))
               .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
            RuleFor(x => x.Code)
               .MustAsync(async (Key, CancellationToken) => !await _discountService.IsCodeExist(Key))
               .WithMessage(_localizer[SharedResourcesKeys.IsExist])
               .When(x => x.Code != 0);
        }
        #endregion
