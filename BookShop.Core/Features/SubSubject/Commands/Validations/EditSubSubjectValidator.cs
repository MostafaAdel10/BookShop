...
{
    public class EditSubSubjectValidator : AbstractValidator<EditSubSubjectCommand>
    {
        #region Fields
        ...
        #region Constructors
        public EditSubSubjectValidator(ISubSubjectService subSubjectService, IStringLocalizer<SharedResources> localizer)
        {
        ...
        {
            RuleFor(x => x.Name_Ar)
                .MustAsync(async (model, Key, CancellationToken) => !await _subSubjectService.IsNameArExistExcludeSelf(Key, model.Id))
                .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
            RuleFor(x => x.Name)
               .MustAsync(async (model, Key, CancellationToken) => !await _subSubjectService.IsNameExistExcludeSelf(Key, model.Id))
               .WithMessage(_localizer[SharedResourcesKeys.IsExist]);
