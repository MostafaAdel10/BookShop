...
namespace BookShop.Service.Implementations
{
    public class ShoppingCartService : IShoppingCartService
    {
        ...
        public async Task<ShoppingCart> AddAsyncWithReturnId(ShoppingCart shoppingCart)
        {
            //Added shoppingCart
        ...
        {
            var shoppingCart = await _shoppingCartRepository.GetTableNoTracking().Where(u => u.ApplicationUserId == userId).FirstOrDefaultAsync();
            return shoppingCart;
        }
        ...
        public async Task<bool> IsShoppingCartIdExist(int id)
        {
            ...
            var shoppingCart = await _shoppingCartRepository.GetTableNoTracking().Where(s => s.Id.Equals(id)).FirstOrDefaultAsync();
            if (shoppingCart == null) return false;
            return true;
        }

        public async Task<bool> UserHasCartAsync(int userId)
        {
            return await _shoppingCartRepository.GetTableNoTracking().AnyAsync(c => c.ApplicationUserId == userId);
        }
        #endregion
    }
}